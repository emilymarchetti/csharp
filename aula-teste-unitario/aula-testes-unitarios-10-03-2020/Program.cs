...
namespace aula_testes_unitarios_10_03_2020
{
    class Program
    {
        ...
        private static IList<ItemMenu> GetMenuItems()
            ...
            {
                new ItemMenu("Login"),
                new ItemMenu("Cadastro Usuário"),
                new ItemMenu("Esqueci a Senha")
            };
        ...
        private static ISubitemMenu GetInstance(ItemMenu item)
        {
            ...
            if (item.Titulo.Equals("Cadastro Usuário"))
                return new UsuarioService(_usuarioRepositorio);
            
            if (item.Titulo.Equals("Esqueci a Senha"))
                return new RecuperarSenhaService(_usuarioRepositorio);
